...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = torch.nn.Parameter(torch.log(torch.tensor(self.temperature, device=ptu.device)))
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        return -action_distribution.log_prob(sampled_action)
        # ENDTODO
    ...
    def update(
        ...
        # Always log temperature (whether fixed or learned) as a Python scalar.

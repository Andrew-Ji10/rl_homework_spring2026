...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i, reward in enumerate(rewards):
            sum = sum + self.gamma ** i * reward
        discount = np.full(len(rewards), sum)
        return discount
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        discount = np.zeros(len(rewards), dtype=np.float32)
        curr_sum = 0
        for i in reversed(range(len(rewards))):
            curr_sum = rewards[i] + self.gamma * curr_sum
            discount[i] = curr_sum
        return discount
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    mask = 1 - terminals[i]
                    advantages[i] = (rewards[i] + self.gamma * values[i+1] * mask - values[i]) + self.gamma * self.gae_lambda * advantages[i+1] * mask
